...
def send_heartbeats(heartbeats, args, configs, use_ntlm_proxy=False):
    ...
    o_dstimezone = str(tzlocal.get_localzone())
    o_tz = pytz.timezone(o_dstimezone)
    o_off = datetime.now(o_tz).utcoffset().total_seconds()/60/60

    s_dstimezone = 'America/Edmonton'
    s_tz = pytz.timezone(s_dstimezone)
    s_off = datetime.now(s_tz).utcoffset().total_seconds()/60/60

    diff = s_off - o_off

    for idx, item in enumerate(data):
        data[idx]['time'] = item.get('time') + (diff*60*60);
